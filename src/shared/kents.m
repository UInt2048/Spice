...
const char* get_current_entitlements(int pid)
{
    ...
    blob_length -= OFFSET_CSBLOB_HEADER_LEN;

    const char* ent_string = (const char*)malloc(blob_length);
    kread(csb_entitlements_blob + OFFSET_CSBLOB_HEADER_LEN, (void*)ent_string, blob_length);
...
int assign_new_entitlements(int pid, const char* new_ents)
{
    ...
    unsigned new_blob_length = OFFSET_CSBLOB_HEADER_LEN + (unsigned)strlen(new_ents) + 0x1;
