...
static int snapshot_rename(const char* path, const char* from, const char* to)
{
    int ret = 0;
...
static const char* get_root_snapshot_name(const char* path)
{
    int dirfd = open(path, O_RDONLY);
...
static kptr_t vnode_from_path(const char* dev_path)
{
...
kern_return_t patch_device_vnode(const char* dev_path)
{
    ...
    kptr_t spec_info = kread_kptr(vnode + OFFSET_VNODE_SPEC_INFO);
    wk32(spec_info + OFFSET_SPEC_INFO_FLAGS, 0); // zero our spec_flags

    return KERN_SUCCESS;
}

kern_return_t dunk_on_mac_mount(void)
{
    kern_return_t ret = 0;
    ...
    uint32_t v_flag = rk32(v_mount + OFFSET_V_MOUNT_V_FLAG);
    if (v_flag == 0x0) {
    ...
    // unset rootfs flag
    wk32(v_mount + OFFSET_V_MOUNT_V_FLAG, v_flag & ~MNT_ROOTFS);
    ...
    v_flag = rk32(v_mount + OFFSET_V_MOUNT_V_FLAG);
    ...
    // set new flags
    wk32(v_mount + OFFSET_V_MOUNT_V_FLAG, v_flag);
