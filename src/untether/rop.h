...
#define CALL_FUNCTION(next_addr, addr, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8) \
    if (rop_var_arg_num != 0 && rop_var_arg_num != -1) {                               \
        LOG("WRONG AMOUNT OF ARGS (line:%d)", __LINE__);                               \
        exit(1);                                                                       \
    }                                                                                  \
    rop_var_arg_num = -1;                                                              \
    ADD_GADGET();                                                                      \
...
#define ROP_VAR_ARG_W_OFFSET(name, nr, offset)                                                        \
    if (rop_var_arg_num == -1) {                                                                      \
        LOG("YOU NEED TO USE ROP_VAR_ARG_HOW_MANY BEFORE USING ROP_VAR_ARG_* (line:%d)", __LINE__);   \
        exit(1);                                                                                      \
    }                                                                                                 \
    rop_var_arg_num--;                                                                                \
